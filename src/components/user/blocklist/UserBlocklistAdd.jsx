...
const UserBlocklistAdd = ({ open, onClose }) => {
  // requirements
  const { t } = useTranslation();

  // api
  const { blocklistAdd, blocklistsGet } = useWazo();
  ...
  const submit = async () => {
    setErrors(null);
    const cleanedNumber = { label: blocklistNumber.label, number: blocklistNumber.number.replace(/\s+/g, "") };
    const res = await blocklistAdd(cleanedNumber);
    if (res.error) {
      ...
      setBlocklistNumber({ label: "", number: "" });
      await blocklistsGet();
      onClose();
      toaster.create({
          title: t("blocklist.success"),
          description: t("blocklist.created"),
          type: 'success',
          duration: 3000,
          closable: true,
        });
...
export default UserBlocklistAdd;
